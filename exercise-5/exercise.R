...
popular_name <- names_2013[names_2013$prop == max(names_2013$prop), "name"]
...
most_popular_in_year <- function(year) {
  that_year <- names[names$year == year,]
  return(that_year[that_year$prop == max(that_year$prop), "name"])
}
...
number_in_million <- function(name, year) {
  ...
  return(name_popularity * 1000000)
}
...
number_in_million("Dinghao", 1997)
